...
class RegistrationDb:
    def __init__(self):
        self.connection = sqlite3.connect("registrationForm.db")
        self.createTable()

    def createTable(self):
        self.connection.execute("CREATE TABLE IF NOT EXISTS STUDENTS(FirstName TEXT, LastName TEXT, DateOfBirth DATE, PlaceOfBirth TEXT, Country TEXT, CourseAppliedFor TEXT, FathersName TEXT, MothersName TEXT, MeanGrade INT)")        
        self.connection.commit()

    def insertRegistrationFormsTable(self, data):
        self.connection.execute("INSERT INTO STUDENTS VALUES(?,?,?,?,?,?,?,?,?)", data)
        self.connection.commit()
